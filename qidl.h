 ...
 *
 * q_str: char* are automatically serialized as strings unless q_size() is
 *   specified (since lacking a q_size() annotation it's understood that it's a
 *   null-terminated string). But in the case of static-allocated char arrays,
 *   where q_size() defaults to number of elements, or where q_size() is specified
 *   explicitly (for whatever reason), but we still want the field serialized as
 *   a string, (as opposed to the default behavior of treating it as an array/list),
 *   the q_str annotation can be specified explicitly.
 *
 * char arrays
 */
...
#define q_string QIDL(serialize, string)
