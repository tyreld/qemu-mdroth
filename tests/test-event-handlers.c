...
static void test_pipe_helper(bool blocking_fd, bool blocking_main_loop)
{
    ...
    pipe_handler_wait_for_write_state(phd, PIPE_HANDLER_STATE_WRITING, EVENT_TIMEOUT);
    pipe_handler_wait_for_read_state(phd, PIPE_HANDLER_STATE_READING, EVENT_TIMEOUT);
    pipe_handler_wait_for_write_state(phd, PIPE_HANDLER_STATE_DONE, EVENT_TIMEOUT);
    pipe_handler_wait_for_read_state(phd, PIPE_HANDLER_STATE_DONE, EVENT_TIMEOUT);
...
static void randomize_data_buffer(char *buf, size_t size)
{
    int i;
...
static bool check_data_buffers(char *buf_out, size_t bytes_written,
                               char *buf_in, size_t bytes_read)
{
    int i;
...
typedef enum SocketStateOffset {
    ...
    SOCKET_STATE_OFFSET_READING_PAUSED,
...
typedef enum SocketState {
    ...
    SOCKET_STATE_READING_PAUSED     = 1 << SOCKET_STATE_OFFSET_READING_PAUSED,
...
typedef struct SocketData {
    ...
    uint32_t caps;
    int fd;
#define SOCKET_DATA_READ_CHUNK 64
#define SOCKET_DATA_WRITE_CHUNK 64
#define SOCKET_DATA_BUF_LEN (128*1024)
    char buf_in[SOCKET_DATA_BUF_LEN];
    char buf_out[SOCKET_DATA_BUF_LEN];
    size_t bytes_written;
    size_t bytes_read;
} SocketData;

/* fuzzy sanity checks, not a real state machine: multiple simultaneous
 * states allowed in some instances, valid transitions depend on
 * individual test cases
 */
static const uint32_t socket_transition_deps[] = {
    [SOCKET_STATE_OFFSET_INIT] = 0,
    [SOCKET_STATE_OFFSET_LISTENING] =
        SOCKET_STATE_INIT,
    [SOCKET_STATE_OFFSET_CONNECTING] =
        SOCKET_STATE_INIT,
    [SOCKET_STATE_OFFSET_CONNECTED] =
        SOCKET_STATE_CONNECTING|SOCKET_STATE_LISTENING,
    [SOCKET_STATE_OFFSET_WRITING] =
        SOCKET_STATE_CONNECTED,
    [SOCKET_STATE_OFFSET_WRITING_COMPLETE] =
        SOCKET_STATE_WRITING,
    [SOCKET_STATE_OFFSET_READING_PAUSED] =
        SOCKET_STATE_CONNECTED,
    [SOCKET_STATE_OFFSET_READING] =
        SOCKET_STATE_CONNECTED,
    [SOCKET_STATE_OFFSET_READING_COMPLETE] =
        SOCKET_STATE_READING,
};

static bool socket_data_state_transition_valid(SocketData *d, uint32_t state,
                                               bool set)
{
    int i;
    ...
    for (i = 0; i < 32; i++) {
        if (((state >> i) & 1) && socket_transition_deps[i]) {
            if (!(d->state & socket_transition_deps[i])) {
                return false;
            }
        }
    }
...
{
    if (!socket_data_state_transition_valid(d, state, set)) {
        return false;
    }
...
static void socket_data_set_caps(SocketData *d, uint32_t caps, bool set)
{
    if (set) {
        d->caps |= caps;
    } else {
...
static bool socket_data_wait_for_state(SocketData *d,
                                       SocketState state)
{
    return socket_data_wait_for_state_timeout(d, state, EVENT_TIMEOUT);
}
...
static ServerData *server_data_new(void)
{
    ...
    sd->addr = g_strdup(strtok(oaddr, ":"));
    sd->port = g_strdup(strtok(NULL, ","));
...
static ClientData *client_data_new(const char *addr, const char *port)
{
    ...
    for (e = res; e != NULL; e = e->ai_next) {
        ...
        if (QEMU_SOCKET_RC_INPROGRESS(rc)) {
            g_assert(socket_data_set_state(d, SOCKET_STATE_CONNECTING, true));
            break;
        } else if (!rc) {
...
static void socket_read(void *opaque)
{
    ...
    g_assert(socket_data_set_state(d, SOCKET_STATE_READING, true));
    ...
    if (ret == 0 || d->bytes_read == SOCKET_DATA_BUF_LEN) {
        ...
        g_assert(socket_data_set_state(d, SOCKET_STATE_READING_COMPLETE, true));
        g_assert(socket_data_set_state(d, SOCKET_STATE_READING, false));
    }
}
...
static void test_socket_registration_atomicity_helper(bool main_loop_blocking,
                                                      bool use_global_mutex)
{
    MainLoop *ml = main_loop_new(main_loop_blocking, use_global_mutex);
    ServerData *sd;
    ClientData *cd;
    ...
    /* event registration/unregistration should be 'atomic' in the
     * sense that we should not see a handler dispatched for an
     * event after we've unregistered it. generally we rely on the
     * global QEMU mutex for this assurance, but if our main loop
     * does not synchronize on a global mutex we should expect to
     * see failures here unless the registration mechanism is
     * itself thread-safe.
     *
     * furthermore, if we aren't synchronizing on the global mutex,
     * the data associated with those event handlers, in this case
     * SocketData objects, must use their own synchronization
     * mechanisms. we can't use both the global mutex *and* the
     * synchronization mechanism for SocketData, however, else we
     * risk an ABBA deadlock due to the iothread/global main lock
     * holding the global mutex while dispatching handlers for
     * our SocketData objects (as opposed to us needing to hold
     * the SocketData mutex prior to taking the global mutex to
     * ensure SocketData state isn't modified while we're
     * determining whether we should [re-]register handlers
     */
    while (true) {
        bool done = false;
        qemu_mutex_lock_iothread();
        if (!(sd->d.state & SOCKET_STATE_READING_COMPLETE)) {
            socket_data_set_caps(&sd->d, SOCKET_CAP_READ, true);
            qemu_set_fd_handler2(sd->d.fd,
                                 socket_can_read, socket_read, NULL, &sd->d);
        }
        if (!(cd->d.state & SOCKET_STATE_WRITING_COMPLETE)) {
            socket_data_set_caps(&cd->d, SOCKET_CAP_WRITE, true);
            qemu_set_fd_handler2(cd->d.fd,
                                 NULL, NULL, socket_write, &cd->d);
        }
        qemu_mutex_unlock_iothread();
        ...
        qemu_mutex_lock_iothread();
        qemu_set_fd_handler2(sd->d.fd, NULL, NULL, NULL, NULL);
        socket_data_set_caps(&sd->d, SOCKET_CAP_READ, false);
        qemu_set_fd_handler2(cd->d.fd, NULL, NULL, NULL, NULL);
        socket_data_set_caps(&cd->d, SOCKET_CAP_WRITE, false);
        if ((sd->d.state & SOCKET_STATE_READING_COMPLETE) &&
            (cd->d.state & SOCKET_STATE_WRITING_COMPLETE)) {
            done = true;
        }
        qemu_mutex_unlock_iothread();
...
static void test_socket_registration_atomicity_main_loop_blocking(void)
{
    test_socket_registration_atomicity_helper(true, true);
}

static void test_socket_registration_atomicity_main_loop_nonblocking(void)
{
    test_socket_registration_atomicity_helper(false, true);
}

#define CONCURRENCY_TEST_REPEAT_COUNT 10
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < CONCURRENCY_TEST_REPEAT_COUNT; i++) {
        sprintf(test_name,
                "/event_handlers/socket/registration_atomicity/main_loop_blocking_%d",
                i);
        g_test_add_func(test_name,
                        test_socket_registration_atomicity_main_loop_blocking);
        sprintf(test_name,
                "/event_handlers/socket/registration_atomicity/main_loop_nonblocking_%d",
                i);
        g_test_add_func(test_name,
                        test_socket_registration_atomicity_main_loop_nonblocking);
    }
